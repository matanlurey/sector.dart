...
void main() {
  test('should find a direct path', () {
    ...
    final path = iterativeDepthFirstSearch.findPath<int>(
      graph,
      1,
      Goal.node(3),
      tracer: TestTracer(expectRepeatedVisits: true),
    );
    check(path).nodesEquals([1, 2, 3]);
  });

  test('should not get stuck in a long branch like standard DFS', () {
    ...
    final path = iterativeDepthFirstSearch.findPath(
      graph,
      'a',
      Goal.node('l'),
  ...
  test('should not find a path when exclusive search is used', () {
    ...
    final path = iterativeDepthFirstSearch.findPathExclusive(
      graph,
      'a',
      Goal.node('a'),
      tracer: TestTracer<String>(),
  ...
  test('should skip a node but have extra visits', () {
    ...
    final tracer = TestTracer<String>(expectRepeatedVisits: true);
    final path = iterativeDepthFirstSearch.findPath(
      graph,
      'a',
      Goal.node('c'),
