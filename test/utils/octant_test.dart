...
void main() {
  group('should convert two points to each $Octant', () {
    test(Octant.first.name, () {
      final a = Pos(0, 0);
      final b = Pos(0, 0);
      check(Octant.between(a, b)).equals(Octant.first);
    });

    test(Octant.second.name, () {
      final a = Pos(3, 1);
      final b = Pos(3, 2);
      check(Octant.between(a, b)).equals(Octant.second);
    });

    test(Octant.third.name, () {
      final a = Pos(3, 1);
      final b = Pos(2, 1);
      check(Octant.between(a, b)).equals(Octant.third);
    });

    test(Octant.fourth.name, () {
      final a = Pos(5, 0);
      final b = Pos(0, 8);
      check(Octant.between(a, b)).equals(Octant.fourth);
    });

    test(Octant.fifth.name, () {
      final a = Pos(0, 0);
      final b = Pos(-5, -2);
      check(Octant.between(a, b)).equals(Octant.fifth);
    });

    test(Octant.sixth.name, () {
      final a = Pos(1, 0);
      final b = Pos(1, -1);
      check(Octant.between(a, b)).equals(Octant.sixth);
    });

    test(Octant.seventh.name, () {
      final a = Pos(-3, 2);
      final b = Pos(0, 0);
      check(Octant.between(a, b)).equals(Octant.seventh);
    });

    test(Octant.eighth.name, () {
      final a = Pos(0, 0);
      final b = Pos(2, -3);
      check(Octant.between(a, b)).equals(Octant.eighth);
    });
  });
